sampleQC <-
  function(data.obj,
           ...
           QCmeasure=c("IQR","ndetectedprobes"),
    ...
    ##-----------------------------------------------------------------------------------------------
    ##-----------------------------------------------------------------------------------------------
    makeplots <- function(expr.dat,goby,xaxis,QC.measure,manualcutoff,mincor,below.smoothed.threshold,labelnote="QC",pch,lw,linecol,thiscor.to,pseudochip.samples,make.legend,...){
    ...
    if(class(data.obj)=="LumiBatch"){
      ...
      if(class(expr.dat)!="matrix") stop("data.obj could not be coerced to a matrix")
    }
    if(logtransform){
      if(min(expr.dat,na.rm=TRUE) <= 0){
        expr.dat <- expr.dat - min(expr.dat,na.rm=TRUE) + 1
      }
      expr.dat <- log2(expr.dat)
    }
    output <- list()
    for (thisxaxis in xaxis){
      for(thiscor.to in cor.to){
        if(identical(class(QCmeasure),"numeric") | identical(class(QCmeasure),"integer")){
          ...
          thismethod <- paste(thisxaxis,thiscor.to,"customQC",sep="_")
          output[[thismethod]] <-
            makeplots(expr.dat,
                      goby,
                      thisxaxis,
                      QC.measure=QCmeasure,
                      thiscor.to=thiscor.to,
        ...
        if("IQR" %in% QCmeasure){
          ...
          thismethod <- paste(thisxaxis,thiscor.to,"IQR",sep="_")
          output[[thismethod]] <-
              makeplots(expr.dat,
                        goby,
                        thisxaxis,
        ...
        if("ndetectedprobes" %in% QCmeasure){
          ...
          thismethod <- paste(thisxaxis,thiscor.to,"ndetectedprobes",sep="_")
          output[[thismethod]] <-
            makeplots(expr.dat,
                      goby,
                      thisxaxis,
                      QC.measure=Raw.fractiondetected,
                      thiscor.to=thiscor.to,
                      pseudochip.samples=pseudochip.samples,
                      manualcutoff,
                      mincor,
